...
class GuestService:
    ...
    def list(self) -> List[Guest]:
        ...
        return sorted([Guest(id,name,inst,role,pov) for (id,name,inst,role,pov,date) in query], key=lambda x:x.id)
    
    def get_for_id(self, id: int) -> Guest|None:
        query = self.db.execute("SELECT * FROM guests WHERE id=?", (id,)).fetchone()
        if query:
            id,name,inst,role,pov, _ = query
            return Guest(id,name,inst,role,pov)
        else:
            return None
    
    def add_entry(self, guest:Guest):
        _,name,institution,role,pov = guest
